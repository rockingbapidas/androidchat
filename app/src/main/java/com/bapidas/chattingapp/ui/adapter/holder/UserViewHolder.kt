 ...
 */
class UserViewHolder constructor(itemView: View) : RecyclerView.ViewHolder(itemView), View.OnClickListener {
    ...
    fun setViewHolder(user: User, clickUser: ClickUser?) {
        if (ChatApplication.applicationContext().id != user.userId) {
            subHolder.visibility = View.VISIBLE
            itemView.visibility = View.VISIBLE
            this.clickUser = clickUser
            userName.text = user.fullName
            emailId.text = user.username
            getLastMessage(user)
        } else {
    ...
    private fun getLastMessage(user: User) {
        checkRooms(user, object : ResultInterface {
