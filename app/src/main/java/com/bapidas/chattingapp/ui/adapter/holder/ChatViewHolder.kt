...
class ChatViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
    private val TAG = ChatViewHolder::class.java.simpleName
    ...
    fun setDataToViews(chat: Chat, isChatContinue: Boolean) {
        when (chat.chatType) {
            Constants.IMAGE_CONTENT ->  {
                ...
                if (chat.fileUrl.isNotEmpty()) {
                    if (isExternalLocal(chat.fileUrl)) {
                        downloadFile(chat)
                    } else {
                        if (chat.senderUid == ChatApplication.applicationContext().id) {
                            val file = File(Uri.parse(chat.fileUrl).path.orEmpty())
                            if (file.exists()) {
                                fileImage?.let { loadPicasso(context, it, chat.fileUrl) }
            ...
            else -> messageText.text = chat.messageText
        }
        userName.text = chat.senderName
        dateTime.text = getTimeAgo(chat.timeStamp)
        if (isChatContinue) {
        ...
        if (chat.senderUid == ChatApplication.applicationContext().id) {
            ...
            if (chat.isSentSuccessfully) {
                if (chat.isReadSuccessfully) {
            ...
            if (!chat.isReadSuccessfully) {
                if (ChatApplication.applicationContext().isChatWindowActive) {
                    updateReadStatus(chat.chatRoom, chat.timeStamp)
                }
            }
        }
    }
    ...
    private fun downloadFile(chat: Chat) {
        val fileHandler = FileHandler()
        val storageReference = ChatApplication.applicationContext().storageInstance
                .getReferenceFromUrl(chat.fileUrl)
        fileHandler.storageReference = storageReference
        val fileName: String
        fileName = if (chat.chatType.contains("image")) {
            storageReference.name
        } else {
            ""
        }
        val filepath: String = if (chat.senderUid == ChatApplication.applicationContext().id) {
            sentPath
        } else {
            receivedPath
        }
    ...
    private fun uploadFile(chat: Chat) {
        val fileHandler = FileHandler()
        val file = Uri.parse(chat.fileUrl)
        fileHandler.storageReference = ChatApplication.applicationContext()
                .chatImageReference.child(getFileName(context, file))
        progressBar?.visibility = View.VISIBLE
        fileHandler.uploadFile(file, chat.chatType, object : FileInterface {
            override fun onProgress(fileModel: FileModel) {
                val res = (fileModel.fileDownloadSnap?.bytesTransferred ?: 0) /
                        (fileModel.fileDownloadSnap?.totalByteCount ?: 0)
                val progress = (100.0 * res)
            ...
            override fun onComplete(fileModel: FileModel) {
                ...
                if (!downloadUrl.isNullOrEmpty()) {
                    ...
                    updateFileUrl(chat.chatRoom, chat.timeStamp, downloadUrl)

                    //push notification after file update is complete
                    chat.fileUrl = downloadUrl
